...
output "transit_vm_host2" {
  description = "Provisioned Transit Vault VM on Host 2"
  value = {
    for k, v in proxmox_virtual_environment_vm.vault_transit : k => {
      id        = v.id
      vm_id     = v.vm_id
      node_name = v.node_name
      ipv4      = split("/", var.transit_vms[k].ip_cidr)[0]
    }
  }
}
